...
class new_reg_branch_laplace(nn.Module):
    def __init__(
        ...
        self.fc_means = nn.Linear(self.embed_dims, self.output_dim)
        self.fc_stds = nn.Linear(self.embed_dims, self.output_dim)
        # self.fc_corr = nn.Linear(embed_dims, self.num_sample)
...
class SparsePoint3DRefinementModule(BaseModule):
    def __init__(
        ...
        self.layers = new_reg_branch_laplace()
    ...
    def forward(
        ...
        reg_out = self.layers(instance_feature + anchor_embed)
