...
class BEVRender:
    ...
    def draw_map_pred(self, result):
        ...
        for i in range(result['scores'].shape[0]):
            ...
            pred_label = [result['labels'][i]]
            pts = result['vectors'][i]
            betas = result['betas'][i]
            x = pts[:, 0]
            y = pts[:, 1]
            betas_x = betas[:, 0]
            betas_y = betas[:, 1]
            # plt.plot(x, y, color=color, linewidth=3, marker='o', linestyle='-', markersize=7)
            self.plot_points_with_laplace_variances(x, y, betas_x, betas_y, color, plt, pred_label, std=True)

    #增加不确定性可视化
    def plot_points_with_laplace_variances(self, x, y, beta_x, beta_y, color, plt, pred_label, std):
        plt.plot(x, y, color=color, linewidth=3, marker='o', linestyle='-', markersize=7)
        ...
        for i in range(len(x)):
            ...
            if pred_label[0]  == 2: #只讲边界线用绿色表示,pred_label为list
                color = 'green'
            else:
                color = color
            ellipse = Ellipse((x[i], y[i]), width=width, height=height,
                                fc=color, lw=0.5, alpha=0.3) #使用不同颜色的椭圆
