...
model = dict(
    ...
    pts_bbox_head=dict(
        ...
        loss_traj=dict(type='L1Loss', loss_weight=0.3),
        loss_traj_cls=dict(
            type='FocalLoss',
            use_sigmoid=True,
            gamma=2.0,
        ...
        loss_map_pts=dict(type='PtsNLLLoss', loss_weight=0.01),     
        ...
        loss_plan_traj_cls=dict(
            ...
            alpha=0.25,
            loss_weight=0.5),
        loss_plan_bound=dict(type='PlanMapBoundLoss', loss_weight=1.5, dis_thresh=1.0),
        loss_plan_col=dict(type='PlanCollisionLoss', loss_weight=1.5),
        loss_plan_dir=dict(type='PlanMapDirectionLoss', loss_weight=1.0)),
...
data = dict(
    ...
    train=dict(
        type=dataset_type,
        data_root=data_root,
        ann_file=ann_data_root + 'vad_nuscenes_infos_temporal_train.pkl',
    ...
    val=dict(type=dataset_type,
             data_root=data_root,
             pc_range=point_cloud_range,
             ann_file=ann_data_root + 'vad_nuscenes_infos_temporal_val.pkl',
             pipeline=test_pipeline,  bev_size=(bev_h_, bev_w_),
             classes=class_names, modality=input_modality, samples_per_gpu=1,
             map_classes=map_classes,
             map_ann_file=ann_data_root + 'nuscenes_map_anns_val.json',
             map_fixed_ptsnum_per_line=map_fixed_ptsnum_per_gt_line,
             map_eval_use_same_gt_sample_num_flag=map_eval_use_same_gt_sample_num_flag,
             use_pkl_result=True,
             custom_eval_version='vad_nusc_detection_cvpr_2019'),
...
load_from = 'ckpt/VAD_modified_base_stage_1.pth'
log_config = dict(
    interval=100,
    hooks=[
...
checkpoint_config = dict(interval=total_epochs, max_keep_ckpts=total_epochs)
