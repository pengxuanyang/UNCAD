...
class MotionPlanningHead(BaseModule):
    def __init__(
        ...
        #map_beta embedding
        self.map_betas_embed = nn.Sequential(
            *linear_relu_ln(embed_dims, 2, 2, input_dims=40),
    ...
    def forward(
        ...
        map_anchors_betas = map_output["prediction_beta"][-1]
        map_anchors_betas_embed = self.map_betas_embed(map_anchors_betas)
        map_instance_feature = torch.stack((map_instance_feature, map_anchors_betas_embed), dim=-1).sum(dim=-1)
        map_instance_feature = self.map_aggr_embed(map_instance_feature)
        ...
        plan_pos = gen_sineembed_for_position(plan_anchor[..., -1, :])  #[6,3,6,256]
        plan_mode_query = self.plan_anchor_encoder(plan_pos).flatten(1, 2).unsqueeze(1) #[6,1,18,256]
