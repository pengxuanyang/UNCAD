...
__all__ = ["MapSparse4DHead"]


@HEADS.register_module()
class MapSparse4DHead(BaseModule):
    def __init__(
        self,
        instance_bank: dict,
        anchor_encoder: dict,
        ...
        **kwargs,
        ...
        super(MapSparse4DHead, self).__init__(init_cfg)
    ...
    def forward(
        ...
        anchor_embed = self.anchor_encoder(anchor)  #[8,100,256]
        ...
        for i, op in enumerate(self.operation_order):
            if self.layers[i] is None:
                ...
                anchor, beta, cls, qt = self.layers[i](
                    instance_feature,
                    anchor,
                    anchor_embed,
                ...
                prediction.append(anchor)   #anchor[8,100,40]
                prediction_beta.append(beta)#beta[8,100,40]
                classification.append(cls)  #cls[8,100,3]
                quality.append(qt)
                if len(prediction) == self.num_single_frame_decoder:
                    instance_feature, anchor, beta = self.instance_bank.update(
                        instance_feature, anchor, beta, cls
                    )
                    if (
        ...
        output.update(
            {
                ...
                "prediction_beta": prediction_beta,  
                "quality": quality,
        ...
        self.instance_bank.cache(
            instance_feature, anchor, beta, cls, metas, feature_maps
        )
    ...
    def loss(self, model_outs, data, feature_maps=None):
        ...
        reg_betas = model_outs["prediction_beta"]  
        quality = model_outs["quality"]
        output = {}
        for decoder_idx, (cls, reg, beta, qt) in enumerate(  
            zip(cls_scores, reg_preds, reg_betas, quality)
        ):
            reg = reg[..., : len(self.reg_weights)]
            beta = beta[..., : len(self.reg_weights)]  
            ...
            beta = beta.flatten(end_dim=1)[mask] 
