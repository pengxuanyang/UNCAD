...
class VADHead(DETRHead):
    ...
    def __init__(self,
        ...
        self.loss_plan_traj_cls = build_loss(loss_plan_traj_cls)
    ...
    def _init_layers(self):
        ...
        map_reg_branch = new_reg_branch_laplace()
        ...
        self.betas_embedding = nn.Sequential(
            nn.Linear(2, self.embed_dims),
            nn.LayerNorm(self.embed_dims),
            nn.ReLU(inplace=True),
            nn.Linear(self.embed_dims, self.embed_dims))
        self.map_aggr_embed = nn.Sequential(
            nn.LayerNorm(self.embed_dims),
            nn.ReLU(inplace=True),
            nn.Linear(self.embed_dims, self.embed_dims),
            nn.LayerNorm(self.embed_dims))                   
        ...
        self.ego_motion_mode_ref = nn.Parameter(
            torch.tensor(ego_motion_mode_ref, dtype=torch.float32),
            requires_grad=False,
        )

        self.ego_motion_mode_embedding_sinemb = nn.Sequential(
            nn.Linear(self.embed_dims, self.embed_dims),
            nn.LayerNorm(self.embed_dims),
            nn.ReLU(inplace=True),
            nn.Linear(self.embed_dims, self.embed_dims),
        ...
        ego_fut_dec_in_dim = self.embed_dims*2 + len(self.ego_lcf_feat_idx) \
            if self.ego_lcf_feat_idx is not None else self.embed_dims*2
        for _ in range(self.num_reg_fcs):
            ego_traj_cls_branch.append(Linear(ego_fut_dec_in_dim, ego_fut_dec_in_dim))
            ego_traj_cls_branch.append(nn.ReLU())
        ego_traj_cls_branch.append(Linear(ego_fut_dec_in_dim, 1))
        self.ego_traj_cls_branch = nn.Sequential(*ego_traj_cls_branch)
    ...
    def forward(self,
        ...
        for lvl in range(map_hs.shape[0]):
            ...
            map_reg_out = self.map_reg_branches[lvl](map_hs[lvl])
            # breakpoint()
            tmp = map_reg_out[:, :, 0:2]
            beta = map_reg_out[:, :, 2:4]

            map_outputs_pts_beta = beta.view(beta.shape[0], self.map_num_vec, self.map_num_pts_per_vec, 2) # [1, 100, 20, 2]
            # breakpoint()
            # TODO: check the shape of reference
            ...
            map_outputs_pts_betas.append(map_outputs_pts_beta)
            
        if self.motion_decoder is not None:
            batch_size, num_agent = outputs_coords_bev[-1].shape[:2]
            # motion_query
            motion_query = hs[-1].permute(1, 0, 2)
            ...
            if self.motion_map_decoder is not None:
                ...
                map_beta = map_outputs_pts_betas[-1]
                map_beta = self.betas_embedding(torch.sqrt(2*(map_beta**2)))
                map_query = torch.stack((map_query, map_beta), dim=-1).sum(dim=-1)
                map_query = self.map_aggr_embed(map_query)
        ...
        map_outputs_pts_betas = torch.stack(map_outputs_pts_betas)
        ...
        # Interaction
        ego_query = ego_his_feats.permute(1, 0, 2)   #[A,B,256]
        ego_motion_mode_ref = self.ego_motion_mode_ref[None].repeat(ego_query.shape[1], 1, 1, 1, 1)

        ego_mode_query = self.gen_sineembed_for_position(ego_motion_mode_ref[..., -1, :])
        ego_mode_query = self.ego_motion_mode_embedding_sinemb(ego_mode_query)
        ego_mode_query = ego_mode_query.flatten(1, 2).permute(1, 0, 2)
        ego_query = (ego_query[:, None, :, :] + ego_mode_query[None]).flatten(0, 1)
        ego_pos = torch.zeros((batch, 1, 2), device=ego_query.device)
        ego_pos_emb = self.ego_agent_pos_mlp(ego_pos)
        ego_pos_emb = ego_pos_emb.unsqueeze(2).repeat(1, 1, self.fut_mode * self.ego_fut_mode, 1).flatten(1, 2)

        ego_query = self.motion_decoder(
                query=ego_query,
                key=ego_query,
                value=ego_query,
        ...
        agent_query = self.agent_fus_mlp(agent_query) # [B, A, fut_mode, 2*D]
        ...
        # Ego prediction
        ego_feats = ego_feats.reshape(ego_feats.shape[0], self.ego_fut_mode, self.fut_mode, ego_feats.shape[-1])
        outputs_ego_trajs = self.ego_fut_decoder(ego_feats)
        outputs_ego_trajs = outputs_ego_trajs.reshape(outputs_ego_trajs.shape[0],
                                                      self.ego_fut_mode, self.fut_mode, self.fut_ts, 2)
        outputs_ego_traj_classes = self.ego_traj_cls_branch(ego_feats)
        outputs_ego_traj_classes = outputs_ego_traj_classes.reshape(outputs_ego_traj_classes.shape[0],
                                                                self.ego_fut_mode, self.fut_mode)
        ...
        outs = {
            ...
            'map_all_pts_betas': map_outputs_pts_betas,
            'enc_cls_scores': None,
            'enc_bbox_preds': None,
            ...
            'ego_traj_cls_scores': outputs_ego_traj_classes
        }
    ...
    def loss_planning(self,
        ...
        ego_traj_cls_scores = ego_traj_cls_scores[ego_fut_cmd==1]
        best_ego_traj_preds = self.get_ego_best_fut_preds(
            ego_fut_preds.reshape(ego_fut_preds.shape[0], self.fut_mode, self.fut_ts, 2),
            ego_fut_gt.reshape(ego_fut_gt.shape[0], self.fut_ts, 2), 
            ego_fut_masks)

        ego_traj_labels = self.get_ego_traj_cls_target(
            ego_fut_preds.reshape(-1, self.fut_mode, self.fut_ts, 2),
            ego_fut_gt.reshape(-1, self.fut_ts, 2),
            ego_fut_masks)

        loss_plan_l1_weight = ego_fut_cmd[..., None, None] * ego_fut_masks[:, None, :, None]
        loss_plan_l1_weight = loss_plan_l1_weight.repeat(1, 1, 1, 2)
        loss_plan_l1_weight = loss_plan_l1_weight[ego_fut_cmd==1]
        ...
        # ego traj classification loss
        ego_traj_cls_scores = ego_traj_cls_scores.reshape(-1, self.fut_mode)
        loss_ego_traj_cls = self.loss_plan_traj_cls(
            ego_traj_cls_scores, ego_traj_labels
        )
        ...
        if digit_version(TORCH_VERSION) >= digit_version('1.8'):
            ...
            loss_ego_traj_cls = torch.nan_to_num(loss_ego_traj_cls)
        ...
        loss_plan_dict['loss_plan_cls'] = loss_ego_traj_cls
    ...
    def get_ego_best_fut_preds(self,
             traj_preds,
             traj_targets,
             gt_fut_masks):
        ...
        dist = dist * gt_fut_masks[:, None, :].repeat(1, self.fut_mode, 1)
        dist = dist[..., -1]
        min_mode_idx = torch.argmin(dist, dim=-1)   #[1]
        min_mode_idx = min_mode_idx[:, None, None, None].repeat(1, 1, self.fut_ts, 2)
        best_traj_preds = torch.gather(traj_preds, 1, min_mode_idx).reshape(-1, self.fut_ts, 2)
    ...
    def get_ego_traj_cls_target(self,
             traj_preds,
             traj_targets,
             gt_fut_masks,
             ):
        ...
        dist = dist[..., -1]
        dist[torch.isnan(dist)] = dist[torch.isnan(dist)] * 0
        traj_labels = torch.argmin(dist, dim=-1)    #[1]
    ...
    def map_loss_single(self,
        ...
        out = torch.cat([pts_preds, pts_betas], dim=-1)
    ...
    def loss(self,
        ...
        map_all_pts_betas = preds_dicts['map_all_pts_betas']
        ...
        ego_traj_cls_scores = preds_dicts['ego_traj_cls_scores']
        ...
        map_losses_cls, map_losses_bbox, map_losses_iou, \
            ...
            map_all_pts_preds, map_all_pts_betas, map_all_gt_bboxes_list, map_all_gt_labels_list,
        ...
        loss_plan_input = [ego_fut_preds, ego_traj_cls_scores, ego_fut_gt, ego_fut_masks, ego_fut_cmd,
    ...
    def get_bboxes(self, preds_dicts, img_metas, rescale=False):
        ...
        for i in range(num_samples):
            ...
            labels = preds['labels']
            trajs = preds['trajs']
            trajs_cls_scores = preds['trajs_cls_scores']
            ...
            map_betas = map_preds['map_betas']

            ret_list.append([bboxes, scores, labels, trajs, trajs_cls_scores, map_bboxes,
                             map_scores, map_labels, map_pts, map_betas])
