...
class PlanningMetric():
    ...
    def reset(self):
        ...
        self.final_obj_box_DAC = torch.zeros(self.n_future)
        self.total = torch.tensor(0)

    def calculate_drivable_area_conflict(self,dataset,idx,pts,trajs, gt_trajs):
        ...
        scores = torch.ones(trajs.shape[-2], dtype=torch.float32)
        gt_scores = torch.ones(trajs.shape[-2], dtype=torch.float32)
        final_scores = torch.ones(trajs.shape[-2], dtype=torch.float32)
        for t in range(trajs.shape[-2]):
            for shift in pts:
                point=Point(trajs[0,t].detach().cpu().numpy()+shift)
                gt_point=Point(gt_trajs[0,t].detach().cpu().numpy()+shift)

                flag=False
                gt_flag=False
                for multi_polygon in info['map_annos'][3][0]:
                    flag = flag or multi_polygon.contains(point)
                    gt_flag = gt_flag or multi_polygon.contains(gt_point)
                scores[t] = scores[t] and flag
                gt_scores[t] = gt_scores[t] and gt_flag
            scores[t]= not scores[t]
            gt_scores[t] = not gt_scores[t]

            if scores[t] == 1 and gt_scores[t] == 1:
                final_scores[t] = 0
            else:
                final_scores[t] = scores[t]  
    ...
    def update(self, index, trajs, gt_trajs, gt_trajs_mask, fut_boxes):
        ...
        final_drivable_area_conflict = self.calculate_drivable_area_conflict(
            self.nuscene_map,
            index,
            self.pts,
            trajs,
        ...
        self.final_obj_box_DAC += final_drivable_area_conflict
    ...
    def compute(self):
        return {
            ...
            'final_obj_box_DAC': self.final_obj_box_DAC / self.total,
...
def planning_eval(results, eval_config, logger):
    ...
    for i, data in enumerate(tqdm(dataloader)):
        ...
        planning_metrics.update(i, pred_sdc_traj[:, :6, :2], sdc_planning[0,:, :6, :2], sdc_planning_mask[0,:, :6, :2], fut_boxes)
