...
class MapInstanceBank(nn.Module):
    def __init__(
        self,
        num_anchor,
        embed_dims,
        ...
        super(MapInstanceBank, self).__init__()
        ...
        self.beta = nn.Parameter(
            torch.tensor(beta, dtype=torch.float32) if beta is not None else torch.ones(self.anchor.shape, dtype=torch.float32),
            requires_grad=False,
        )
    ...
    def get(self, batch_size, metas=None, dn_metas=None):
        ...
        beta = torch.tile(self.beta[None], (batch_size, 1, 1))  
        ...
        if (
            ...
            if self.anchor_handler is not None:
                ...
                self.cached_beta = self.anchor_handler.anchor_projection(  
                    self.cached_beta,
                    [T_temp2cur],
                    time_intervals=[-time_interval],
                )[0]
    ...
    def update(self, instance_feature, anchor, beta, confidence):
        if self.cached_feature is None:
            return instance_feature, anchor, beta
        ...
        if instance_feature.shape[1] > self.num_anchor:
            ...
            beta = beta[:, : self.num_anchor]  
            confidence = confidence[:, : self.num_anchor]

        N = self.num_anchor - self.num_temp_instances
        confidence = confidence.max(dim=-1).values
        _, (selected_feature, selected_anchor, selected_beta) = topk(
            confidence, N, instance_feature, anchor, beta
        )
        ...
        if num_dn > 0:
            ...
            beta = torch.cat([beta, dn_beta], dim=1) 
        return instance_feature, anchor, beta
    ...
    def cache(
        ...
        (
            self.confidence,
            (self.cached_feature, self.cached_anchor, self.cached_beta),
        ) = topk(confidence, self.num_temp_instances, instance_feature, anchor, beta)
